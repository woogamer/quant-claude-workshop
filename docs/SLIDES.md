...
명령 한 줄에 Claude 가 어떤 도구를 어떻게 호출하는지.
...
**Claude 응답** (도구 자동 선택)
```
[도구 호출] get_stock_price("005930")
→ 삼성전자(005930) 현재가: 292,500원
  전일대비 −7,000원 (−2.34%)
  거래량 18,194,628주
```
...
**Claude 응답**
```
[도구 호출] buy_stock("005930", 1)
→ 모의계좌 005930 1주 시장가 매수 접수
  체결가 292,500원 · 잔고 4,707,500원 차감
  (한국장 마감 시간이면 다음 영업일 시초가 체결)
```

→ 핵심: 사용자는 도구 이름·종목코드 외울 필요 없음. "삼성전자" 만 말하면 됨.

---

## 시연 ② — 알고리즘 신호 → 판단 → 매수

신호를 먼저 보고, 결과에 따라 다음 행동 결정.
...
**Claude 응답** (도구 자동 선택)
```
[도구 호출] golden_cross_signal("NVDA")
→ NVDA · 종가 $219.51 (2026-05-21)
  MA5 $222.25 · MA20 $214.39 · MA5 > MA20 (정배열)
  신호: HOLD (교차 없음 — 추세 유지)
```
...
**Claude 응답** (조건부 판단)
```
HOLD 신호라 강한 매수 시그널은 아닙니다.
정배열 유지 중이고 5일선이 20일선 위 — 추세는 양호.
그래도 진행하시겠습니까? (사용자 확인 필요)

> 응 매수해줘
[도구 호출] buy_overseas_stock("NVDA", 1)
→ NVDA 1주 시장가 매수 접수
```

→ 핵심: LLM 이 "신호 안 좋다" 라고 솔직히 말하고 **사용자에게 확인 받음** (CLAUDE.md 매매 원칙).

---

## 시연 ③ — DART 공시 → 내부자 매수 종목 → 매수

여러 도구 chain — 발굴부터 매수까지.
...
**Claude 응답** (도구 2번 호출)
```
[도구 호출] dart_recent_disclosures(days=3)
→ 최근 3일 99건 공시 수신

[도구 호출] insider_buying_signals(disclosures)
→ 임원·주요주주 매수 시그널 (다중 공시 순):
  · 이노스페이스 — 7건 (소유상황·거래계획 다수)
  · 오텍 — 4건
  · 대한광통신 — 2건
  · SKC, 바이넥스, 하림지주 — 각 1건
```
...
**Claude 응답**
```
[도구 호출] buy_stock("462350", 1)
→ 이노스페이스(462350) 1주 시장가 매수 접수
  공시 7건 = 임원·주요주주가 짧은 기간에 반복 매수/거래계획
  주가 변동성 큰 종목이라 유의
```

→ 핵심: Claude 가 도구 2개를 chain (조회 → 필터 → 매수). 사용자는 "공시 보고 매수" 한 줄만.

...
**① 컨센서스만 보면 결국 대형주 쏠림**
액티브 ETF 35개가 동시에 담는 종목 = 삼전·하이닉스. "이미 다 아는 종목" 만 나옴.
→ 최근 출시 ETF에 가중 + 시간감쇠로 신선한 시그널 우대 → 187개 후보까지 분산

**② 토스 인기순위로 단타 신호 만들기**
가설 — "갑자기 사람들이 토스에서 많이 보는 종목" 이 단기 모멘텀 신호일까?
거래량 폭발 + 토스 인기 상승 → 5분마다 자동 스캔, 페이퍼 매매로 가설 검증 (다음 슬라이드)
...
1주차 결과: 패시브 종목 안정성 95% (현행 85%) — BUT 3일 짧은 페이퍼에선 **−1.86%p** 짐 (로보틱스 28% sectoral 집중).
